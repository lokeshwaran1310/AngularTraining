...
export class BugService{
    private apiUrl:string="http://localhost:8080/bugs/all";
    ...
    getBugById(id: number): Observable<Bug> {
        return this.http.get<Bug>(`http://localhost:8080/bugs/title/${id}`).pipe(
            catchError((error) => {
    ...
    searchBugs(title?: string, status?: string, assignee?: string): Observable<Bug[]> {
        ...
        const searchUrl = `http://localhost:8080/bugs/search?${params.toString()}`;
